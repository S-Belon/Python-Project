...
with c1:
    st.markdown('### Heatmap')
    heatmap_figure = graphs.heat_map(MAIN.cme_df)
...
st.pyplot(halfangle_figure)  # Display the Matplotlib figure using st.pyplot()

speed_figure = graphs.ts_speed(MAIN.weekly_averages)
st.pyplot(speed_figure)  # Display the Matplotlib figure using st.pyplot()
